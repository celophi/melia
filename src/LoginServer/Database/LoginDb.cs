...
namespace Melia.Login.Database
{
	public class LoginDb : MeliaDb
	{
		...
		public bool CheckUpdate(string updateFile)
		{
			...
			{
				var query = @"SELECT * FROM `updates` WHERE `path` = @path";
				var result = conn.Query<string>(query, new { path = updateFile }).FirstOrDefault();
				return result != null;
			}
		}
		...
		/// <summary>
		/// Returns an account that matches the name.
		/// </summary>
		...
		public bool SetAccountPassword(string accountName, string password)
		{
			...
			using (var conn = this.GetConnection())
			{
				var query = @"UPDATE `accounts` SET `password` = @p_password WHERE `name` = @p_name";
				return conn.ExecuteScalar<bool>(query, new { p_teamName = password, p_name = accountName });
			}
		}
	}
}
